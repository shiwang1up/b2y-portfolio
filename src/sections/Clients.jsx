...
const Clients = () => {
  ...
  return (
    <section
      ref={carouselRef}
      className="py-11 text-black overflow-hidden relative  bg-gradient-to-br from-primary-600  to-primary-800"
    >
      <div className="absolute inset-0 bg-[url('../assets/UI/Curve_Line.svg')] bg-[length:100%_100%] bg-no-repeat bg-center opacity-20"></div>
        ...
        <div className="flex items-center justify-center  bg-white shadow-lg py-5">
          ...
          >
            {duplicatedClients.map((client, index) => (
              <motion.div
                key={index}
                className="flex-shrink-0 px-32  transition-opacity "
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                <img src={client.logo} alt={client.name} className="h-28 " />
              </motion.div>
            ))}
          </motion.div>
        </div>
